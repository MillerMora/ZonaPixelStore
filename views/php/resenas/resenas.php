...
if ($busqueda !== '') {
    $consulta = buscar_resenas($busqueda);
} else {
            ...
            <div class="dashboard-card-header d-flex align-items-center justify-content-between">
                ...
                <form method="GET" style="display: contents;">
                    <div class="dash-search-wrap position-relative">
                        <input type="search" name="q" class="dash-search" value="<?php echo htmlspecialchars($busqueda); ?>" placeholder="Buscar reseñas..." autocomplete="off" />
                    ...
                    <tbody>
                        <?php while ($filas = mysqli_fetch_assoc($consulta)): ?>
                            <tr>
                                ...
                                <td class="text-accent-strong"><?php $producto = consultar_producto_id($filas['producto_id']);
                                                                echo $producto ? htmlspecialchars($producto['nombre']) : 'Producto no encontrado'; ?></td>
                                <td>
                                    <span class="pill-muted">
                                        <?php $usuario = consultar_usuarios_id($filas['autor_id']);
                                        echo $usuario ? htmlspecialchars($usuario['nombre']) : 'Autor no encontrado'; ?>
                                    </span>
                                </td>
                                <td><?= $filas['titulo'] ?></td>
