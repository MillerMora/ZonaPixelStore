...
if ($busqueda !== '') {
    $consulta = buscar_productos($busqueda);
} else {
            ...
            <div class="dashboard-card-header d-flex align-items-center justify-content-between">
                ...
                <form method="GET" style="display: contents;">
                    <div class="dash-search-wrap position-relative">
                        <input type="search" name="q" class="dash-search" value="<?php echo htmlspecialchars($busqueda); ?>" placeholder="Buscar productos..." autocomplete="off" />
                    ...
                    <tbody>
                        <?php while ($filas = mysqli_fetch_assoc($consulta)): ?>
                            <tr>
                                ...
                                <td><?= $filas['precio'] ?></td>
                                <td><?= $filas['stock'] ?></td>
                                <td><?php if ($filas['destacado'] == 1) {
                                        echo 'Si';
                                    } else {
                                        echo 'No';
                                    } ?></td>
                                <td><?php if ($filas['activo'] == 1) {
                                        echo 'Activo';
                                    } else {
                                        echo 'No Activo';
                                    } ?></td>
